...
\section{Dynamic Semantics}
    ...
    % TODO: how to specify initial condition, for x and x', which need additionally coincide, have same subdivision
    ...
    \begin{proof}
        ...
        % FIXME: more possible variables, common subdivision is cup
        ...
        Let $c\in\mathcal{K}$ and $\zeta\in(\hat{t}_j,\hat{t}_{j+1})$. By definition, $\hat{t}_j=t_i-\hat{c}$ for some $i\in\set{\range{0}{m}}$ and $\hat{c}\in\mathcal{K}$.
        ...
        If $c+\zeta\geq 0$, then $\trajectory(\zeta)(x)(s)$ is differentiable in $\zeta$ with $\trajectory(\zeta)(\D{x})(s) = \DD{\trajectory(t)(x)(s)}{t}(\zeta)$ by the semantics of DDEs (Definition~\ref{def:semantic-HP}(\ref{itm:sem-HP-DDE})).
        % FIXME: jumppoints and pw conitnuity
