...
\section{Syntax}
    ...
    Let $\allvars$ be the set of \textit{all variables} and $\diffvars\defeq\{\D{x} \with x\in\allvars\}$ the corresponding set of \textit{differential symbols}.
    %and set of \textit{delayed variables} $\delayedvars\defeq\{\xtau \with x\in\allvars\}$
    ...
    \begin{definition}[Terms]
        ...
        \textit{Terms} are defined by the following grammar, extending classical \dL with a symbol for a \textbf{delayed variable}:
        \begin{equation}
        ...
        The delay $s\geq 0$ must be non-negative. For $\xtau[0]$ we write $x$.
    ...
    \begin{definition}[Hybrid program]
        ...
        \textit{Hybrid programs} are defined by
        ...
        The formula $\bsfml$ is of first-order logic of real arithmetic (\FOLR$(\allvars)$).
    \end{definition}
    ...
    \begin{definition}[\ddL formula]
        ...
        \begin{align}
            \asfml,\bsfml \Coloneqq
                &\lforall{s\in[-\tau,s]}{\astrm\geq\bstrm} \mid
                \lforall{s\in[-\tau,s]}{p(\range{\istrm{1}}{\istrm{k}})} \mid\\
        ...
        $\tau$ is a symbolic constant related to the history duration induced by the occurence of a delay-differnetial equation. Its value is set by proof rules.
