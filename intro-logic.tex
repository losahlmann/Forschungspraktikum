\chapter{Introduction to Logic}
    ...
    \section{First-Order Logic}
        ...
        \subsection{Syntax}
            ...
            First-order logic inductively defines a syntax of \textit{logical formulas} from a set of function and predicate symbols, the \textit{signature} $\signature$ of the theory.

            alphabet to built well-formed formulas from
            a set of symbols and their roles

            The signature comprises three subsets: predicates, functions and constants.

            nullary function

            The syntax only defines how the given symbols can be textually combined. At this stage, they have no specific meaning yet.

            \begin{definition}[Terms]
                Well-formed terms are either logical variables, constants or functions applied to terms:
                The set of all \textit{terms} $\terms$ is the smallest set with
                \begin{enumerate}
                    \item If $x\in\varsymbols$ then $x\in\terms$.
                    \item If $c\in\signature$ is a term
                    % TODO: command for integer range
                    \item If $f\in\signature$ of arity $n>0$ and $\istrm{i}\in\terms$ for $i=\range{1}{n}$, then $f(\range{\istrm{1}}{\istrm{n}})\in\terms$
                \end{enumerate}
                ...
                \end{equation}
                where $x$ is ranging over the set of logical variables $\varsymbols$, $c$ over the nullary functions in $\signature$ and $f$ over the elements in $\signature$ with arity $n>0$.
            \end{definition}
            ...
            \begin{definition}[First-Order Formulas]
                ...
                smallest set with
                \begin{enumerate}
                    \item If predicate symbol $p\in\signature$ of arity $n\geq 0$ and $\istrm{i}\in\terms$ for $i=\range{1}{n}$ then $p(\range{\istrm{1}}{\istrm{n}})\in\FOLformulas$
