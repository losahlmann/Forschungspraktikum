...
\section{Piecewise Continuous Functions}
    ...
    \begin{lemma}
        chain of continuous and piecewise-continuous is again piecewise-continuous with same subdivision
    \end{lemma}
...
\section{Existence and Uniqueness of Solutions}
    ...
    \begin{proof}
        ...
        By the the fundamental theorem of calculus is $x$ continuous on $\compactum{\tzero}{\tzero+T}$ and differentiable in $t\in\compactum{\tzero}{\tzero+T}$ if $s\mapsto f(s,x(s),x(s-\tau_1),\ldots,x(s-\tau_k))$ is continuous in $s=t$.
         ...
         has the subdivision
        We define a subdivision of $\compactum{\tzero}{\tzero+T}$ by
        \begin{equation*}
            \mathcal{Z}\defeq\set{\tzero,\tzero+T}\cup\bigcup_{j=1}^{k}\bigcup_{\substack{i=1\\t_i\geq\tau_j}}^{m}\set{t_i-\tau_j}\defeq\subdivision{\hat{t}_0}{\hat{t}_p}
        \end{equation*}
