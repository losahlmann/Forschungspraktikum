...
\section{Definition of Solution}
    ...
    \begin{definition}[Solution of DDE]\label{def:solution-dde}
        ...
        % FIXME: restrict to each subdivision interval, fullfills deriv there
        % FIXME: and is continuous
...
\section{Existence and Uniqueness of Solutions}
    ...
    \begin{definition}[Lipschitz Continuity]\label{def:lipschitz}
        ...
        % FIXME: dont I need xtau in right side ??? L(\nnorm*{x-x}+\nnorm*{y-y})
    ...
    \end{lemma}
    \begin{proof}
        => x solution of DDE, ie pw cont, hence f(x) integrable

        <= x solution of int eq
        Let $x\from [\tzero-\tau,\tzero+T]$ be a solution of the integral equation~\eqref{eq:integral-equation}, i.e. $x(t)=x_{\tzero}(t)$ for all $t\in\compactum{\tzero-\tau}{\tzero}$ and $x(t) = x_{\tzero}(\tzero) + \int_{\tzero}^t f(s,x(s),x(s-\tau))\dx[s]$ for $t\in\compactum{\tzero}{\tzero+T}$ for a $T>0$.
        HS -> continuous
        since f cont, x cont -> integrand cont iff xtau cont
        let t_0<...<tm be the subdivision of init cond
        HS -> F diffbar on (t_i,t_{i+1}) with x'=F'=f()
    ...
    \begin{proof}
        ...
        This proofs the existence of a solution to the DDE.
