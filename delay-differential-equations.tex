...
\begin{lemma}[]
    ...
    A piecewise continuous function, as defined in Definition \ref{def:piecewise-continuous} is (Riemann) integrable.
\end{lemma}

\begin{proof}
    See standard analysis literature, such as \cite{Rudin76PrinciplesAnalysis} or \cite{gathmanngrundlagen}.
...
\begin{equation}
    \gamma_x:[0,T]\rightarrow\statespace[\tau],\\ t\mapsto\xbartaut{t}
\end{equation}
...
\begin{lemma}
    ...
    %
    % Then $t \mapsto x_t = x(t+\theta)$, where $\theta\in[-\tau,0]$, is a piecewise continuous mapping from $[\sigma,\sigma+T]$ into $\statespace[\tau]$.
\end{lemma}
...
% TODO: on [\sigma,t_1] DDE equiv to ODE/IntEq
% -> ex unique sol on [\sigma, t_1]
% -> ex unique sol on [\sigma,\tau] (glob Lip of f on [sigma,tau])
% -> ex unique sol on [\sigma,2\tau] (continuous?, diffable?)
